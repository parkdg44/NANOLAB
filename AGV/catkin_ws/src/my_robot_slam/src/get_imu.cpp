...
geometry_msgs::Twist yaw_val, yaw_kal;
...
int main(int argc, char **argv)
{
	...
	yaw_pub = nh.advertise<geometry_msgs::Twist>("yaw_val", 1);
    yaw_kal_pub = nh.advertise<geometry_msgs::Twist>("yaw_kal", 1);
	...
	while (ros::ok())
	{
		...
		if (sPort != -1)
		{
			...
			else
			{
				...
				if (fabs(d_theta) <= 0.00015) d_theta = 0;
				yaw_past = yaw;
				...
				ROS_INFO("HPF :  %.4f Yaw : %.4f, alpha = %.4f", -theta*180/M_PI, -yaw*180/M_PI, tau / (tau + dt));
					...
					//yaw_val.yaw = -theta;
					//yaw_kal.yaw = -d_theta;
				yaw_val.angular.z = -theta;
				yaw_kal.angular.z = -theta;
